...
**One `program` model.** Anything an app needs beyond a plain install is one optional `program` field on its catalog entry (`AppProgram` in `src/lib/catalog/types.ts`), with three kinds: `commands` (declarative shell, `auto` to run silently post-install), `panel` (bespoke React UI by `id` in `setup/registry.ts`), and `sandboxed` (a partner's setup program, below). Use the `isPanelProgram()` guard: `panel` and `sandboxed` both open a modal and can `handlesInstall`/`revertOnUninstall`, as opposed to declarative `commands`. `AppSetupPanel` routes by kind. Anything non-standard is a program: custom per-app sources were removed, and morphe (modded apps) runs as a first-party program too (`catalog/programs/morphe/program.js`, run headlessly via `src/lib/programs/morphe-runner.ts`) using the native `portal.verifyMorpheManifest` capability so the Ed25519 key stays on the host.

## Sandboxed setup programs

Some apps (Immortal is the reference consumer) need full device setup, not just an install (disable Meta's package verifier, grant `REQUEST_INSTALL_PACKAGES`, fix the Gen-1 installer dialog, set launcher and screensaver). A `program: { kind: "sandboxed", repo, trust }` catalog entry declares this. The procedure is **not hand-ported**: it is a JavaScript **program** the partner ships in their repo (fetched live from their release; a built-in default at `catalog/apps/immortal-launcher/program.js` is the offline fallback for Immortal, a 1:1 of its `provision.sh`). OpenPortal runs it in a **sandboxed Web Worker** with no DOM, no credential store, and no live `Adb` handle; the program drives the device only through a constrained `portal` capability API that RPCs to a **broker** on the main thread (`src/lib/programs/{worker,broker,loader,types}.ts`, public barrel `index.ts`). The broker holds the `Adb` handle, validates every call (https-only, path/flag/package allowlists), and surfaces an audit log + kill switch. One generic panel (`src/components/apps/setup/SandboxedProgramPanel.tsx`) runs every sandboxed program, its form driven by a **manifest** the program declares, so new questions need no front-end change. The loader reads `repo`/`programPath`/`trust` from the catalog entry (not hardcoded to Immortal) and **only fetches/runs `verified` or `first-party` programs** — `trust` is enforced at PR-review time. The host/program contract is versioned (`PORTAL_API_VERSION` in `src/lib/programs/types.ts`); the SDK for program authors lives in `sdk/`. Config values come from a typed live view of `config.env` (`src/lib/programs/config.ts`, parameterized by repo) with a vendored snapshot under `catalog/apps/immortal-launcher/upstream/` (Biome-ignored) as fallback and drift baseline. `scripts/check-provision-drift.mjs` (CI: `.github/workflows/provision-drift.yml`) alerts when upstream `provision.sh` or `openportal.program.js` moves; `scripts/vendor-provision.mjs` re-vendors in one shot. Full detail and the fidelity map live in `docs/programs.md`.
